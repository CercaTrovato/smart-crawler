...
<PY> -m scrapling install      # 下 Chromium + patchright 浏览器
```
...
## 注意（实测踩过的坑）
- **本机 Clash fake-ip 代理会黑洞"直连抓取"**（Rust/Go 类客户端超时到保留段 IP）→ config `network.bypassProxyForFetch=true`（默认开），Playwright 也传 `proxy=direct://`。**唯独 Firecrawl 的 API 调用要走代理**（`FIRECRAWL_PROXY`）。
